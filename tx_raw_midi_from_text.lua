    ...
    return function()
        ...
        function portlist()
            ...
            for p in t[2]:iter() do
                ...
                if amp:isnil() or not amp:sends_output() then
                    goto continue
                end
                rv[amp:name()] = amp
        ...
        local dialog_options = {{
            ...
            key = "autoinc",
            default = false,
            title = "Add F0 and F7 bytes?"
        }, {
            type = "entry",
            key = "syxstring",
            title = "Enter SysEx bytes (up to 256B)"
        }, {
            type = "dropdown",
            key = "port",
            title = "Target Port",
        ...
        if (string.len(rv["syxstring"]) > 256) then
            LuaDialog.Message("Raw MIDI Tx",
                "Only supports sending up to a 256 byte message. Use a SYX file instead for longer messages.",
