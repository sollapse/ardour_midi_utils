...
function factory(params)
    ...
    return function(signal, ref, ...)
        ...
        function chkConfig()
            ...
            if ARDOUR.LuaAPI.file_test(cfgname, ARDOUR.LuaAPI.FileTest.Exists) then
                ...
                if configured then
                    ...
                    for i = 0, 127 do
                        noteoff_buf = noteoff_buf ..
                                          string.char(
                                (0x80 & 0xF0) + tonumber(string.format("%x", mpk2_padchn - 1), 16),
                                tonumber(string.format("%x", i), 16), 0x00)
                    end
                else
                    if debug then
                        print("MPK2 Auto Feedback not configured.")
                    end
                    return
                end
            end
        ...
        function configFeedback()
            if debug then
                print("Multiple selections: " .. tostring(multi_sel) .. " Size: " ..
                          tostring(sel.tracks:routelist():size()))
            end
            ...
            for t in Session:get_tracks():iter() do
                ...
                if not miditrk:isnil() and not padfdbk:isnil() and miditrk:is_selected() and #miditrk:comment() > 0 then
                    ...
                    if not miditrk:name():find(padfdbk_name) then
                        ...
                        if b:find("BANK") then
                            if debug then
                                print("Selected MIDI Track: " .. miditrk:name() .. " Bank tag: " .. b)
                            end
                            ...
                            local syx = string.char(0xf0, 0x47, 0x00, mpk2_model, 0x30, 0x00, 0x04, 0x01, 0x00, 0x18,
                                mpk2_bank, 0xf7)
                            -- transmit message
        ...
        function selectTrack()
            ...
            elseif sel.tracks:routelist():size() > 1 then
                ...
                if falsecnt == 0 then
                    ...
                    if debug then
                        print("Multiple selections do not share the same bank. Last track selected in use.")
                    end
                    multi_sel = false
                    configFeedback()
                end
