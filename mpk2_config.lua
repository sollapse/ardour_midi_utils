...
function factory () 
	...
	function deviceList()
		_, v = Session:engine():get_backend_ports ("", ARDOUR.DataType("midi"), ARDOUR.PortFlags.IsInput | ARDOUR.PortFlags.IsPhysical, C.StringVector())
		p = {}
	...
	if allvars then 
		dialog_options = {
			{ type = "entry", key = "fdbktrack", default = padfdbk_name, title = "Pad Feedback Track Name" },
			{ type = "checkbox", key = "hidetrack", default = false, title = "Hide Feedback Track?" },
			{ type = "dropdown", key = "port", default = Session:engine():get_pretty_name_by_name(mpk2_out), title = "Select MPK2 Device", values = deviceList()},
			{ type = "entry", key = "padchn", default = mpk2_padchn, title = "Pad Channel" }
		}
	else
		--use default values
		dialog_options = {
			{ type = "entry", key = "fdbktrack", default = "MPK2 Pad Fdbk", title = "Pad Feedback Track Name" },
			{ type = "checkbox", key = "hidetrack", default = false, title = "Hide Feedback Track?" },
			{ type = "dropdown", key = "port", title = "Select MPK2 Device", values = deviceList()},
			{ type = "entry", key = "padchn", default = "10", title = "Pad Channel" }
		}
	end
	--run main dialog
	...
	if t:isnil() then 
		...
		padfdbk = Session:new_midi_track(
					ARDOUR.ChanCount(ARDOUR.DataType ("midi"), 1),
					ARDOUR.ChanCount(ARDOUR.DataType ("midi"), 1),
					true,
					ARDOUR.PluginInfo(),nil,
					nil, 1, padfdbk_name, ARDOUR.PresentationInfo.max_order, ARDOUR.TrackMode.Normal,false)
